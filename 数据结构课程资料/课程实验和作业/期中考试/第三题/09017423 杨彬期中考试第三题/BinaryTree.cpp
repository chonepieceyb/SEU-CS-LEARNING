...
int BinaryTree::getDegreeOne(BinTreeNode* subTree)
{
	if (subTree == NULL)   //如果是空数的话，递归结束的条件
	{
		return 0;
	 }
	if (subTree->leftChild != NULL && subTree->rightChild == NULL) //如果只有左子树没有右子树
	{
		return 1 + getDegreeOne(subTree->leftChild);  //递归调用
	}
	else if (subTree->leftChild == NULL && subTree->rightChild != NULL)   //如果只有右子树，没有左子树的话
	{
		return 1 + getDegreeOne(subTree->rightChild);   //递归调用
	}
	else    //如果都不是 也就是说这个节点并不是度为1的节点
	{
		return getDegreeOne(subTree->rightChild) + getDegreeOne(subTree->leftChild);
	}
}