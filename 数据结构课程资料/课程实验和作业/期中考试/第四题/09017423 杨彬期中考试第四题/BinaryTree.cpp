...
int BinaryTree::getWeightLength(BinTreeNode* subTree, int length)
{
	if (subTree == nullptr)  //如果是空节点的话返回0;
	{
		return 0;
	}
	if (!(subTree->leftChild== nullptr && subTree->rightChild == nullptr))  //如果不是叶子节点
	{
		return getWeightLength(subTree->leftChild, length + 1) + getWeightLength(subTree->rightChild, length + 1);
	}
	else
	{
		cout << "叶节点" << subTree->data << "权值" << static_cast<int>(subTree->data)<<endl;
		return static_cast<int>(subTree->data) *length;    //返回权值乘以路径长
